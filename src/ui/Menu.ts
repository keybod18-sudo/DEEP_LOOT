...
function equipmentPreviewHtml(item: WeaponItem | ArmorItem): string {
  ...
  return `<span class="equipment-preview" style="display:block;margin-top:14px;padding:15px;border-radius:22px;border:1px solid ${border};background:linear-gradient(135deg,rgba(7,11,18,.99),rgba(15,11,22,.99));box-shadow:inset 0 0 52px rgba(0,0,0,.5),0 12px 28px rgba(0,0,0,.25);">
    <span style="display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,.92fr);gap:16px;align-items:stretch;">
      <span style="display:flex;flex-direction:column;justify-content:space-between;padding:17px 18px;border-radius:18px;border:1px solid rgba(255,255,255,.07);background:linear-gradient(180deg,rgba(255,255,255,.028),rgba(255,255,255,.01));min-height:350px;">
        <span>
          <span style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:15px;">
            <span style="font-size:27px;line-height:1.1;font-weight:800;color:rgba(255,255,255,.98);">${label}</span>
            <span style="padding:6px 10px;border-radius:999px;border:1px solid ${border};color:${accent};font-size:12px;font-weight:700;white-space:nowrap;">${escapeHtml(rarity)}</span>
          </span>
          <span style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:17px;">
          ...
          <span style="display:block;font-size:12px;letter-spacing:.13em;color:rgba(255,255,255,.5);margin-bottom:7px;">固有能力</span>
          ...
          <span style="display:block;font-size:14px;line-height:1.72;color:rgba(255,255,255,.91);">${ability}</span>
        </span>
      </span>
      <span style="display:flex;align-items:center;justify-content:center;padding:10px;border-radius:18px;border:1px solid ${border};background:#080b12;min-height:350px;overflow:hidden;">
        ${artSrc ? `<img src="${artSrc}" alt="${label}" style="width:100%;height:100%;max-height:380px;object-fit:contain;display:block;border-radius:14px;" />` : `<span style="color:rgba(255,255,255,.45);">IMAGE NOT FOUND</span>`}
      </span>
    </span>
  </span>`;
}
